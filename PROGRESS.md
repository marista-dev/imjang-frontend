...
- [ ] LoginPage (로그인)
- [ ] SignupPage (회원가입)
- [ ] VerifyEmailPage (이메일 인증 - OTP 6자리)
- [ ] 인증 플로우 전체 빌드 확인 + 리뷰 + 커밋
...
## Phase 3: 홈 페이지
- [ ] HomePage (환영 섹션 + 통계 + 퀵 액션 + 최근 매물)
- [ ] BottomNav를 App.jsx에 통합
- [ ] 빌드 확인 + 리뷰 + 커밋
...
## Phase 4: 타임라인
- [ ] TimelinePage (날짜 그룹 + 무한 스크롤 + PropertyCard)
- [ ] 빌드 확인 + 리뷰 + 커밋
...
- [ ] Step 1: 사진 업로드 (ImageUploader 연동)
- [ ] Step 2: 위치 정보 (카카오 주소 검색 API)
- [ ] Step 3: 가격 정보 (거래 유형별 동적 폼)
- [ ] Step 4: 기본 정보 (면적, 층수, 별점, 가격 평가)
- [ ] Step 5: 체크리스트 (토글, 선택 버튼, 멀티 선택 칩)
- [ ] Step 6: 메모 + 제출
- [ ] PropertyNewPage 통합 + 빌드 확인 + 리뷰 + 커밋
...
## Phase 6: 매물 상세
- [ ] PropertyDetailPage (이미지 갤러리, 정보, 체크리스트, 환경 태그, 주변 시설, 메모)
- [ ] 삭제 기능 (ConfirmModal 연동)
- [ ] 빌드 확인 + 리뷰 + 커밋
...
## Phase 7: 매물 수정
- [ ] PropertyEditPage (기존 데이터 로드 + 수정 폼)
- [ ] 이미지 추가/삭제 기능
- [ ] 빌드 확인 + 리뷰 + 커밋
...
## Phase 9: 최종 마무리
- [ ] 전체 라우팅 점검 (모든 페이지 이동 확인)
- [ ] 전체 빌드 최종 확인 (`npm run build`)
- [ ] 불필요 코드/주석/console.log 정리
- [ ] README.md 작성
