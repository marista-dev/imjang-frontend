    ...
    <div className="relative">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4">
        <div className="flex items-center gap-1 mb-1">
          ...
          <p className="text-sm text-white/90 truncate">{property.address}</p>
        </div>
        <p className="text-xl font-bold text-white leading-tight">
          {renderHeroPrice(property)}
        </p>
        <div className="mt-1.5 flex items-center gap-2 flex-wrap">
          <RatingStars rating={property.rating ?? 0} readOnly size="sm" colorOverride="text-amber-400" />
    ...
    <div className="min-h-screen bg-white pb-24">
      ...
      <div className="px-5 pt-4">
        ...
        <button
          type="button"
          onClick={handleShare}
          className="flex h-10 w-full items-center justify-center gap-2 rounded-full border border-slate-200 text-sm font-medium text-slate-600 active:bg-slate-50 transition-colors"
        >
          <LinkIcon size={15} />
          공유하기
        </button>
          ...
          <Section icon={Store} title="편의시설">
            <div className="space-y-3">
              {amenities.map((a) => {
                ...
                return (
                  <div key={a.categoryCode} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-500 truncate">
                        {a.nearestName}
                        <span className="ml-1 text-slate-400">도보 {walkMin}분</span>
                      </p>
                    </div>
                  </div>
                );
        ...
        {/* 체크리스트 검토 섹션 */}
        <Section icon={ClipboardList} title="체크리스트 검토">
          <div className="space-y-2.5">
            {/* 즉시 입주 가능 */}
            <div className="flex items-center gap-2">
              {property.canMoveIn ? (
                <Check size={16} className="flex-shrink-0 text-success" />
              ) : (
                <XIcon size={16} className="flex-shrink-0 text-danger" />
              )}
            ...
            {property.parkingType && PARKING_LABELS[property.parkingType] && (
              <div className="flex items-center gap-2">
                <Car size={16} className="flex-shrink-0 text-slate-500" />
                <span className="text-sm text-slate-700">
          ...
          <button
            ...
            className="mt-4 flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-slate-200 text-sm font-medium text-slate-600 active:bg-slate-50 transition-colors"
          >
            <Edit3 size={15} />
            체크리스트 수정하기
          </button>
          ...
          <Section icon={MapPin} title="주변환경">
            <div className="flex flex-wrap gap-2">
              {environments.map((env) => (
                <span
                  key={env}
                  className="rounded-full bg-primary-50 px-3 py-1.5 text-xs font-medium text-primary border border-primary/20"
                >
